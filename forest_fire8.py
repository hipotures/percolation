...
class ForestFireSimulationGPU:
    ...
    def initialize_grids(self):
        ...
        self.grids = cp.random.choice(
            [1, 0],
            size=(self.num_simulations, self.size, self.size),
            p=[self.p_values[:, None, None], 1 - self.p_values[:, None, None]]
        ).astype(cp.int8)

        # Losowy początek płomienia dla każdej symulacji
        initial_fire = cp.random.randint(0, self.size, size=(self.num_simulations, 2))
        self.grids[cp.arange(self.num_simulations), initial_fire[:, 0], initial_fire[:, 1]] = 2
    ...
    def process_epoch(self):
        ...
        self.simulation_finished |= ~still_burning

    def run_simulation(self):
        """
        Uruchamia symulacje aż do zakończenia wszystkich pożarów.

        :return: Liczba epok dla każdej symulacji
        """
