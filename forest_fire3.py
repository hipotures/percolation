...
class ForestFire:
    def __init__(self):
        ...
        self.red_objects = []
        self.epochs_in_last_sec = 0  # Liczba epok w bieżącej sekundzie
        self.epochs_per_second = 0   # Epoki na sekundę
        self.last_time = time.time()  # Czas ostatniej aktualizacji epok na sekundę
    ...
    def run(self):
        while True:
            ...
            if self.red_objects:
                ...
                self.epochs_in_last_sec += 1  # Zwiększ licznik epok
            else:
                # Jeśli symulacja się zakończyła, nie zwiększamy liczby epok
                pass

            # Aktualizacja epok na sekundę
            current_time = time.time()
            if current_time - self.last_time >= 1.0:
                self.epochs_per_second = self.epochs_in_last_sec
                self.epochs_in_last_sec = 0
                self.last_time = current_time

            # Aktualizacja tytułu okna z informacjami
            if self.red_objects:
                pygame.display.set_caption(
                    f'RedMuts - P: {self.probability:.2f} - E: {self.current_epoch} - R: {len(self.red_objects)} - S: {self.epochs_per_second}e/s'
                )
            else:
                pygame.display.set_caption(
                    f'RedMuts - P: {self.probability:.2f} - E: {self.current_epoch} - R: 0 (finished) - S: {self.epochs_per_second}e/s'
                )
