...
def run_multiple_simulations(p_values, runs=10, size=500, min_epochs=100, batch_size=10):
    """
    ...
    :param batch_size: Liczba symulacji uruchamianych jednocześnie
    :return: Mediany liczby epok i czasy dla każdej wartości P
    """
    num_ps = len(p_values)
    med_epochs = np.zeros(num_ps)
    med_times = np.zeros(num_ps)
    ...
    for i, p in enumerate(p_values):
        ...
        if len(epochs_list) == 0:
            ...
            med_epochs[i] = np.median(epochs_list)
            med_times[i] = np.median(times_list)
            print(f"P = {p:.6f}: Mediana epok = {med_epochs[i]:.2f}, Mediana czasu = {med_times[i]:.4f} s")
    return med_epochs, med_times
...
def save_results(filename, iteration, p_values, med_times, med_epochs):
    """
    ...
    :param med_times: Lista median czasów
    :param med_epochs: Lista median epok
    """
    with open(filename, 'a', encoding='utf-8') as f:
        f.write(f"Iteracja {iteration}:\n")
        f.write("P\tMediana czasu [s]\tMediana liczba epok\n")
        for p, t, e in zip(p_values, med_times, med_epochs):
...
def main():
    ...
    runs_per_p = 1000  # Zwiększenie liczby powtórzeń
    ...
    min_epochs = 10  # Minimalna liczba epok do uwzględnienia
    ...
    current_p_max = p_end
    best_p_history = []
    consecutive_stable = 0  # Licznik stabilnych iteracji

    for it in range(1, max_iterations + 1):
        print(f"\n--- Iteracja {it} ---")
        p_values = generate_p_values(current_p_min, current_p_max, steps)
        med_epochs, med_times = run_multiple_simulations(p_values, runs=runs_per_p, size=size, min_epochs=min_epochs, batch_size=10)

        # Zapis wyników do pliku
        save_results(wynik_file, it, p_values, med_times, med_epochs)

        # Znalezienie P z maksymalną medianą epok
        max_time_idx = np.argmax(med_times)
        best_p = p_values[max_time_idx]
        best_time = med_times[max_time_idx]
        best_epoch = med_epochs[max_time_idx]
        print(f"Najlepsze P w iteracji {it}: {best_p:.6f} z medianą epok = {best_epoch:.2f} i medianą czasu = {best_time:.4f} s")

        # Dodanie best_p do historii
        best_p_history.append(best_p)

        # Sprawdzenie warunku zakończenia: czy best_p jest stabilne w ostatnich 3 iteracjach
        if len(best_p_history) >= 3:
            if (np.isclose(best_p_history[-1], best_p_history[-2], atol=1e-6) and
                np.isclose(best_p_history[-2], best_p_history[-3], atol=1e-6)):
                print(f"Znaleziono stabilną wartość P = {best_p:.6f} w trzech kolejnych iteracjach. Zakończenie programu.")
                break

        # Sprawdzenie minimalnego rozmiaru przedziału
        step_size = (current_p_max - current_p_min) / (steps - 1)
        new_step = step_size / 3  # Zmniejszenie tempa zawężania
