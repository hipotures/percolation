...
def run_multiple_simulations(p_values, runs=10, size=500):
    ...
    for i, p in enumerate(p_values):
        ...
        print(f"P = {p:.6f}: Średnia epok = {avg_epochs[i]:.2f}, Średni czas = {avg_times[i]:.4f} s")
...
def main():
    ...
    steps = 50
    runs_per_p = 10
    max_iterations = 10  # Maksymalna liczba iteracji podziału
    size = 100
    wynik_file = "wyniki_symulacji.txt"
    min_step = 1e-6  # Minimalny rozmiar przedziału P
    ...
    for it in range(1, max_iterations + 1):
        ...
        # Sprawdzenie warunku zakończenia: czy best_p zmienił się
        if previous_best_p is not None:
            # Używamy funkcji numpy.isclose dla porównania z tolerancją
            if np.isclose(best_p, previous_best_p, atol=1e-6):
                print(f"Znaleziono stałą wartość P = {best_p:.6f} w dwóch kolejnych iteracjach. Zakończenie programu.")
                break

        previous_best_p = best_p
