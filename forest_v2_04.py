...
class ForestFire:
    def __init__(self):
        self.size = 100  # Grid size (100x100). Możesz zwiększyć do 1000x1000, ale pamiętaj o wydajności.
    ...
    def run_simulations(self):
        ...
        for idx, point in enumerate(self.tree_positions):
            ...
            if epoch > self.best_epoch:
                ...
                self.best_grid = copy.deepcopy(burned_trees)
                print(f"New best point found at {point} with {epoch} epochs.")
                self.visualize_current_best(cluster_index)
    ...
    def visualize_current_best(self, cluster_index):
        """
        Updates the display to show only the current best simulation's burned cluster.

        :param cluster_index: Index of the current best cluster
        """
        self.screen.fill((255, 255, 255))  # Clear screen with white background

        # Draw all clusters up to the current best
        for idx, cluster in enumerate(self.burned_clusters):
